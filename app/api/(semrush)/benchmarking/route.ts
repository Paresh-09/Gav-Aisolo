...
export async function GET() {
  try {
    ...
    const bechmarkingUrl = `https://api.semrush.com/?type=domain_organic&key=${SEMRUSH_API_KEY}&display_filter=%2B%7CPh%7CCo%7Cseo&display_limit=10&export_columns=Ph,Po,Pp,Pd,Nq,Cp,Ur,Tr,Tc,Co,Nr,Td&domain=seobook.com&display_sort=tr_desc&database=us`;

    // const response = await axios.get(bechmarkingUrl)
    const response = `Keyword;Position;Previous Position;Position Difference;Search Volume;CPC;Url;Traffic (%);Traffic Cost (%);Competition;Number of Results;Trends
...
seo book keyword suggestion tool free download;1;1;0;70;0.00;http://tools.seobook.com/;0.29;0.00;0.14;775000;0.29,1.00,0.00,0.43,0.00,0.00,0.14,0.00,0.14,0.00,0.00,0.43`;
    ...
    const sheetId = await createSpreadSheet("Bechmarking Report", parsedData);
    ...
    console.error("Error generating bechmarking report", error);
    return NextResponse.json(
      {
        success: false,
        error: "Failed to generate bechmarking report",
      },
      { status: 500 }
    );
  }
