...
import { createSpreadSheet, updateSpreadSheet } from "@/app/utils/googleSheets";
import { parseCSV } from "@/app/utils/parseCSV"; // Ensure this is imported

const SPREADSHEET_ID = process.env.SPREADSHEET_ID; // Store your spreadsheet ID in an environment variable

export async function GET(req: Request) {
  try {
    // Temporary data to simulate SEMrush API response
    const mockData = `Keyword;Position;Previous Position;Position Difference;Search Volume;CPC;Url;Traffic (%);Traffic Cost (%);Competition;Number of Results;Trends
...
seo book keyword suggestion tool free download;1;1;0;70;0.00;http://tools.seobook.com/;0.29;0.00;0.14;775000;0.29,1.00,0.00,0.43,0.00,0.00,0.14,0.00,0.14,0.00,0.00,0.43`;
    ...
    if (SPREADSHEET_ID) {
      ...
      await updateSpreadSheet(SPREADSHEET_ID, parsedData);
      spreadsheetId = SPREADSHEET_ID;
    } else {
      // Create a new spreadsheet
      spreadsheetId = await createSpreadSheet("SEMrush Data", parsedData);
    }
    ...
    console.error("Error creating or updating spreadsheet:", error);
    return new Response(
      JSON.stringify({ error: "Failed to create or update spreadsheet" }),
      {
        status: 500,
        headers: {
          "Content-Type": "application/json",
